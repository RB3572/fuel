...
/// Raw candidate patterns for "Learn from me" (see learning.js) — food/weekday
/// clustering, what a workout does to the next night's sleep and HRV, workout timing
/// consistency, and notably more active weekdays. The client hands these to the model
/// to pick from and phrase; nothing here is generated by AI.
async function handleLearningSignals(req, res) {
  if (req.method !== 'GET') {
    methodNotAllowed(res, ['GET'])
    return
  }
  ...
  try {
    const auth = await authenticatedUser(req)
    if (!auth) {
      sendJson(res, 401, { error: 'Sign in to see your patterns.' })
      return
    }
    const signals = await getLearningSignals(auth.id)
    sendJson(res, 200, signals, auth.cookie ? [auth.cookie] : [])
  } catch (error) {
    console.error('Learning signals request failed', error)
    sendJson(res, 500, { error: 'Unable to find patterns right now.' })
  }
}
