...
type GoalKey = 'calories'|'calorieBalancePercent'|'protein'|'carbs'|'fat'|'fiber'|'move'|'exercise'|'stand'|'steps'|'sleepHours'|'restingCaloriesFloor'
...
export default function App(){
  ...
  const sectionNodes:Record<SectionKey,{title:string;detail:string;node:ReactNode}>={
    ...
    vitals:{title:'Vitals',detail:'Cardiovascular, oxygen, and respiratory measures',node:<><section className="metric-grid"><Metric icon={<HeartPulse/>} label="Resting heart rate" value={s?.restingHeartRate} unit="bpm"/><Metric icon={<Activity/>} label="HRV" value={s?.hrv} unit="ms"/><Metric icon={<Activity/>} label="Respiratory rate" value={s?.respiratoryRate} unit="/min" decimals={1}/><Metric icon={<Activity/>} label="VO₂ max" value={s?.vo2Max} unit="mL/kg/min" decimals={1}/>{positive(s?.bloodOxygen)&&<Metric icon={<Activity/>} label="Blood oxygen" value={s?.bloodOxygen} unit="%" decimals={1}/>}{positive(s?.walkingHeartRateAverage)&&<Metric icon={<HeartPulse/>} label="Walking heart rate" value={s?.walkingHeartRateAverage} unit="bpm avg"/>}{positive(s?.cardioRecovery)&&<Metric icon={<HeartPulse/>} label="Cardio recovery" value={s?.cardioRecovery} unit="bpm" decimals={1}/>}</section><VitalTrends data={data?.trends||[]}/></>},
...
function GoalsSetup({initial,onClose,onSaved}:{initial:DashboardData|null;onClose:()=>void;onSaved:()=>Promise<void>|void}){
  ...
  useEffect(()=>{fetch('/api/goals',{cache:'no-store'}).then(r=>r.json()).then((p:GoalApiResponse)=>{if(Number.isFinite(p.calorieBalancePercent)){setGoals({calorieBalancePercent:p.calorieBalancePercent,protein:p.protein,carbs:p.carbs,fat:p.fat,fiber:p.fiber,move:p.move,exercise:p.exercise,stand:p.stand,steps:p.steps,sleepHours:p.sleepHours,restingCaloriesFloor:p.restingCaloriesFloor});setReference({averageExpenditure:p.averageExpenditure,calories:p.calories,days:p.averageExpenditureDays})}}).catch(()=>{})},[])
  const submit=async()=>{setBusy(true);setMessage('');try{const r=await fetch('/api/goals',{method:'PUT',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({goals})}),p:GoalApiResponse&{error?:string}=await r.json();if(!r.ok)throw new Error(p.error||'Unable to update goals.');setGoals({calorieBalancePercent:p.calorieBalancePercent,protein:p.protein,carbs:p.carbs,fat:p.fat,fiber:p.fiber,move:p.move,exercise:p.exercise,stand:p.stand,steps:p.steps,sleepHours:p.sleepHours,restingCaloriesFloor:p.restingCaloriesFloor});setReference({averageExpenditure:p.averageExpenditure,calories:p.calories,days:p.averageExpenditureDays});setMessage('Goals saved.');await onSaved()}catch(e){setMessage(e instanceof Error?e.message:'Unable to update goals.')}finally{setBusy(false)}}
  ...
  ] as Array<[EditableGoalKey,string,string]>).map(([key,label,unit])=><label className="goal-field" key={key}><span>{label}<small>{unit}</small></span><input type="number" min="0" step={key==='sleepHours'?.1:1} value={goals[key]} onChange={e=>setGoals(g=>({...g,[key]:Number(e.target.value)}))}/></label>)}</div><div className="auto-goals"><div className="auto-goals-head"><HeartPulse size={18}/><div><h3>Minimum resting calories</h3><p>If a day's synced resting calories end up below this once the day is over, Fuel bumps that day's resting (and total) calories up to this floor so it doesn't understate that day's burn. Set to 0 to turn this off.</p></div></div><label className="goal-field"><span>Resting floor<small>kcal</small></span><input type="number" min="0" max="3000" step="10" value={goals.restingCaloriesFloor} onChange={e=>setGoals(g=>({...g,restingCaloriesFloor:Number(e.target.value)}))}/></label></div><button className="save-goals-button" disabled={busy} onClick={()=>void submit()}><Save size={17}/>Save goals</button>{message&&<p className="sync-message">{message}</p>}<p className="sync-note">The calorie target updates automatically as the average of completed Apple Health expenditure days changes.</p></section></div>
}
...
// Vitals over time. The tiles above answer "what is it today"; these answer "is it
// drifting", which is the only way a resting heart rate or a VO2 max is readable at
// all. Only metrics with at least two days of history get a chart, so a sparsely
// synced signal leaves a gap rather than an empty box.
const VITAL_TRENDS:Array<{key:keyof TrendPoint;title:string;unit:string;yLabel:string;decimals?:number}>=[
  {key:'restingHeartRate',title:'Resting heart rate',unit:'bpm',yLabel:'Beats per minute'},
  {key:'hrv',title:'HRV',unit:'ms',yLabel:'Milliseconds'},
  {key:'vo2Max',title:'VO\u2082 max',unit:'mL/kg/min',yLabel:'mL/kg/min',decimals:1},
  {key:'walkingHeartRateAverage',title:'Walking heart rate',unit:'bpm',yLabel:'Beats per minute'},
  {key:'respiratoryRate',title:'Respiratory rate',unit:'/min',yLabel:'Breaths per minute',decimals:1},
  {key:'bloodOxygen',title:'Blood oxygen',unit:'%',yLabel:'Percent',decimals:1},
  {key:'cardioRecovery',title:'Cardio recovery',unit:'bpm',yLabel:'Beats per minute',decimals:1},
]
function VitalTrends({data}:{data:TrendPoint[]}){
  const charts=VITAL_TRENDS.filter(v=>data.filter(p=>typeof p[v.key]==='number').length>=2)
  if(!charts.length)return null
  return <section className="panel vital-trends">
    <h3>Vitals over time</h3>
    <div className="vital-trend-grid">
      {charts.map(v=><div className="vital-trend" key={String(v.key)}>
        <InteractiveLine data={data} metric={v.key} unit={v.unit} decimals={v.decimals||0} chartTitle={v.title} yLabel={v.yLabel}/>
      </div>)}
    </div>
  </section>
}
...
function SignIn(){return <main className="center"><div className="signin"><h1>Fuel</h1><p>Your private nutrition, activity, and recovery dashboard.</p><button onClick={()=>location.assign('/api/auth/google/start')}><ShieldCheck size={18}/>Sign in with Google</button><button className="apple-signin" onClick={()=>location.assign('/api/auth/apple/start')}><svg viewBox="0 0 384 512" width="16" height="16" aria-hidden="true" fill="currentColor"><path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z"/></svg>Sign in with Apple</button><p className="signin-note">Either account works — Fuel matches you by email address.</p></div></main>}
